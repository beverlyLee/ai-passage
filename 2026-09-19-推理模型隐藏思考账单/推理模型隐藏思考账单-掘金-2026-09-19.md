...
> 系列第 27 篇。读者默认是有经验、会调 API 的程序员。全文不编造版本号与价格。文中两类数字来源不同：**价格、发布时间线、论文结论**来自官方一手文档与公开研究；**量级演示数字**（如 11.6×、过度思考掉点）来自文末一个纯标准库、可注入假后端的**模拟脚本**的实跑输出——脚本本机就能跑、自带 `--self-test`，但你 clone 下来看到的是「模拟推理模型」的数字，不是真实 o1/o3 跑出来的。
...
我把计费明细拉出来才发现问题不在接口，在模型本身。他每次请求虽然只拿回一小段可见文本，但账单上「输出 token」那栏记的是可见文本加上一段他根本看不到的东西——推理模型在背后走的那条思考链（thinking tokens）。那段思考链占了计费输出的绝大部分，而且他没法关掉它，也没法在返回里看到它。
...
把 `billed_output = visible + hidden` 这行注释掉改成只算 visible，账单立刻少掉一大块（模拟器里能少约 96%，但这是 toy 参数结果，不是 o3 实测）——但现实里你改不了厂商的计费逻辑，你能改的只有「少触发隐藏思考」和「别把它用在不需要的地方」。
...
这是最反直觉的一条，也是为什么「想得久 = 更准」这个前提在工程上会塌房。这条结论不是我用模拟器编的——多篇公开发表的研究已经实测出来：

- **arXiv 2506.04210**（马里兰等，2025-06）：在 GSM-8K 上，思考 token 约 1100 个时准确率达到峰值 **87.3%**，但堆到约 15980 个时反而掉到 **70.3%**。
- **arXiv 2510.07880**（TRACE，Google DeepMind + 密歇根大学，2025-10）：在简单任务上多耗 **5–20×** token，却没有实质精度增益。
- **arXiv 2507.04023v2**（Srivastava 等，2025-07）：覆盖 53 个 LLM、14 个基础数学任务，推理模型平均多耗约 **18×** token，有时精度反而更低；constrained 设定下甚至崩塌 **28–36%**。

换句话说，推理模型的准确率随思考预算呈**非单调曲线**：先升后降，不是越多越好。
...
下面结合论文的同一结论，用 `02_overthinking.py` 的 toy 参数把这三条曲线跑给你看（注意：以下数字是模拟器示意值，不是 o1/o3 实测，只为复现“越过峰值就掉”的形状）：
...
回看标题——推理模型的账单里藏着一段你既看不到、也关不掉、还照收钱的内部思考——四个踩坑其实是同一个根因的三条分支加一个控制面：
